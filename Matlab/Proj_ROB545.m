...
% During the move motion, keep the cylinder can level at all time to avoid spill. Specify an additional constraint on the interim manipulator configurations for the RRT planner. Turn the constraint on by setting the UseConstrainedSampling property to true.
            ...
            close all
            file_name = sprintf('Test_2_InitObj env%d planner%d itr%d-RRT-Star-Itr-limit.csv' , a, b, c);
            file_name2 = sprintf('Test_2_ObjGoal env%d planner%d itr%d-RRT-Star-Itr-limit.csv' , a, b, c);
...
save('Test_2_env-1-6_planner-1-2_avg-10_without-display-RRT-Star-Itr-limit.mat', 'time_to_execute')
save('Test_2_individual-execution-time_env-1-6_planner-1_without-display-RRT-Star-Itr-limit.mat', 'execution_time') 
